...
def load_data(filename):
    ...
    with open(filename, encoding='utf-8') as f:
        for l in f:
            content,title = l.strip().split('fengefu')
            D.append((content,title))
    return D


# 加载数据集
train_data = load_data('./data/data_train/train.csv')
valid_data = load_data('./data/data_train/valid.csv')
...
autotitle = AutoTitle(start_id=None, end_id=tokenizer._token_end_id, maxlen=30)
...
if __name__ == '__main__':
    ...
    testdata = []
    with open('./data/data_train/test.csv', encoding='utf-8') as f:
        for l in f:
            content = l.strip()
    ...
    for line in testdata:
        line = line.strip()
        line_x = line.replace('[图片]', "")
        line_x = line_x.replace('[语音]', "")
        line_x = line_x.replace('|', "")
        line_x = line_x.replace('\t', "")
        line_x = line_x.replace(' ', "")
        topk = 1
        pred_title = ' '.join(autotitle.generate(line_x, topk))
        print('*'*20)
        print("inputs:",line)
        print("results: ",pred_title)
