...
def split_input(xs, fre,ys, gpu_nums):
    """
    split input
    :param xs: articles
    :param ys: summaries
    ...
    fre = [tf.split(x, num_or_size_splits=gpu_nums, axis=0) for x in fre]
    ys = [tf.split(y, num_or_size_splits=gpu_nums, axis=0) for y in ys]

    return [(xs[0][i], xs[1][i]) for i in range(gpu_nums)], [(fre[0][i], fre[1][i]) for i in range(gpu_nums)],[(ys[0][i], ys[1][i], ys[2][i]) for i in range(gpu_nums)]